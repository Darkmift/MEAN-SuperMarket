...
class UserCtrl {
  ...
  static async getLastActiveCart(req, res, next) {
    try {
      ...
      // const getCarts = await Cart.find({ customerRef: id }, function(
      //   err,
      //   docs
      // ) {
      //   if (err) {
      ...
      const getCarts = await Cart.find({ customerRef: id })
        .sort({ dateEdited: -1 })
        .limit(1)
        .lean();

      console.log("TCL: getCarts", getCarts);

      // lastCart = await getCarts
      //   .sort({ dateEdited: -1 })
      //   .exec(function(err, doc) {
      //     if (err) throw err;
      //     var dateEdited = doc["dateEdited"];
      //     console.log(dateEdited); // sorting_index == 10
      //   });
      ...
      return res
        .status(200)
        .json({ message: /*lastCart ||*/ getCarts || "oopsie?", id: id });
