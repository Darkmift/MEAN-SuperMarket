...
class CartItemController {
  static async createOrUpdate(req, res, next) {
    try {
      ...
      const itemExist = await CartItem.findOne({ uniqueName })
        .lean()
        .exec();
      ...
      if (itemExist) {
        ...
        if (updatedItem) {
          const postUpdateItem = await CartItem.findOne({ uniqueName })
            .lean()
            .exec();
      ...
      const refProduct = await Product.findById(productRef)
        .lean()
        .exec();
  ...
  static async getOne(req, res, next) {
    ...
    console.log('TCL: CartItemController -> getOne -> _id', _id);
    try {
      var singleCartItem = await CartItem.findById(_id).exec();
      res.status(200).send({
